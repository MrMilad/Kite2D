...
USA
*/
#include "Kite/Core/memory/kstackallocator.h"

namespace Kite {
	KStackAllocator::KStackAllocator(SIZE Size, void *Start)
	...
	KStackAllocator::~KStackAllocator() {
#if _DEBUG
		_kprevPos = nullptr;
#endif
	...
	void* KStackAllocator::allocate(SIZE Size, U8 Alignment) {
		KDEBUG_ASSERT(Size != 0);
	...
	void KStackAllocator::deallocate(void *Pointer) {
