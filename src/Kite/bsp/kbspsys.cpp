...
namespace Kite {
	...
	void KGCullingSys::reset(KNode *Hierarchy, KEngine *Engine) {
		_kqtree->Clear();
		delete _kobjpool;
		_kobjpool = new memory::memory_pool<>(sizeof(KQTreeObject), KGCULLING_MEM_CHUNK * sizeof(KQTreeObject));

		// recursive scan hierarchy
		scan(Hierarchy->getRoot());
	}

	void KGCullingSys::scan(KNode *Root) {

	}
	
	bool KGCullingSys::update(F64 Delta) {
	...
	void KGCullingSys::nodeAdded(KNode *Node) {

	}

	void KGCullingSys::nodeRemoved(KNode *Node) {

	}


	void KGCullingSys::componentAdded(KComponent *Com) {

	}

	void KGCullingSys::componentRemoved(KComponent *Com) {
	...
	void KGCullingSys::_switchType(KCullable *Interface) {
		if (Ent->getStatic()) {
			// remove from bf list
			_removeBFObject(Com);
			...
			// add to dynamic list
			_addToBFList(Interface);
		}
	}

	void KGCullingSys::_computeParentPosition(KNode *Node, KRectF32 &Output){
