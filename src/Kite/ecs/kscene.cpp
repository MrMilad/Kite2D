...
namespace Kite {
	lua_State *KScene::_klstate = nullptr;
	bool KScene::initeLua() {
	...
	bool KScene::renameEntity(const KHandle &EHandle, const std::string &NewName) {
	...
	void KScene::removeEntity(KHandle Handle) {
	...
	void KScene::removeEntityByName(const std::string &Name) {
		auto found = _kentmap.find(Name);
		if (found != _kentmap.end()) {
	...
	KEntity *KScene::getEntity(const KHandle &Handle) {
		auto ent = _kestorage.get(Handle);
		return ent;
	}

	KEntity *KScene::getEntityByName(const std::string &Name) {
		auto found = _kentmap.find(Name);
		if (found != _kentmap.end()) {
			return getEntity(found->second);
		}
	...
	U32 KScene::recursiveSaveChilds(KEntity *Entity, KPrefab *Prefab, U32 Level, bool Name, bool CopyPrefab) {
		auto id = Level;
		// entity information
		if (Name) {
	...
	bool KScene::createPrefab(const KHandle &EHandle, KPrefab *Prefab, bool SaveName){
		Prefab->clear();
		auto ent = _kestorage.get(EHandle);
		if (ent == nullptr) {
	...
	KHandle KScene::loadPrefabRaw(KPrefab *Prefab, bool isPaste) {
		// check prefab
	...
	KHandle KScene::loadPrefab(KPrefab *Prefab) {
		return loadPrefabRaw(Prefab, false);
	}

#ifdef KITE_EDITOR
	bool KScene::copyPrefab(const KHandle &EHandle, KPrefab *Prefab, bool Name) {
		Prefab->clear();
		auto ent = _kestorage.get(EHandle);
		if (ent == nullptr) {
			KD_PRINT("an entity with the given handle does not exist.");
	...
	KHandle KScene::pastePrefab(KPrefab *Prefab) {
