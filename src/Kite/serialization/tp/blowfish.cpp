...
size_t PKCS5PaddingLength(const std::vector<unsigned char> &data) {
  if (data.empty())
    return 0;
  unsigned char length = data[data.size() - 1];
...
Blowfish::Blowfish(const std::vector<unsigned char> &key) {
  SetKey(key.data(), key.size());
}

void Blowfish::SetKey(const unsigned char *key, size_t byte_length) {
...
void Blowfish::Encrypt(std::vector<unsigned char> &Data) {
  size_t padding_length = Data.size() % sizeof(uint64_t);
  if (padding_length == 0) {
  ...
  for (size_t i = 0; i < padding_length; ++i) {
	  Data.push_back(static_cast<char>(padding_length));
  }

  for (unsigned int i = 0; i < Data.size() / sizeof(uint64_t); ++i) {
    uint32_t *left = &reinterpret_cast<uint32_t *>(Data.data())[i * 2];
    uint32_t *right = &reinterpret_cast<uint32_t *>(Data.data())[i * 2 + 1];
    EncryptBlock(left, right);
  }
}

void Blowfish::Decrypt(std::vector<unsigned char> &Data) {
  for (unsigned int i = 0; i < Data.size() / sizeof(uint64_t); ++i) {
    uint32_t *left = &reinterpret_cast<uint32_t *>(Data.data())[i * 2];
    uint32_t *right = &reinterpret_cast<uint32_t *>(Data.data())[i * 2 + 1];
  ...
  size_t padding_length = PKCS5PaddingLength(Data);
